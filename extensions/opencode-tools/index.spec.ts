...
describe('OpenCode adapter', () => {
  it('registers the configuration tools', async () => {
    ...
    expect(Object.keys(hooks.tool ?? {})).toEqual(['config_create', 'config_get', 'issue_id']);
  ...
  it('delegates execution to the generic configuration tools', async () => {
    ...
    try {
      ...
      const issueId = await tools['issue_id']?.execute({ prompt: 'Please investigate issue 00042' }, context);
