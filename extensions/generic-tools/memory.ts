...
export type RecordType = 'fact' | 'decision' | 'event' | 'lesson';
export type SourceKind = 'artifact' | 'user-confirmed' | 'discussion' | 'tool-observation';
export type Confidence = 'confirmed' | 'verified';
...
const TYPE_PAIRS: Record<MemoryType, RecordType[]> = {
  semantic: ['fact'],
  episodic: ['decision', 'event'],
  procedural: ['lesson'],
};
...
function validateRecord(value: unknown, settings: MemorySettings): asserts value is MemoryRecord {
  if (!isMapping(value)) throw new MemoryError('Memory record must be a mapping.');
  const record = value as unknown as MemoryRecord;
  if (record.schema_version !== 1) throw new MemoryError('Unsupported memory record schema version.');
  assertUlid(record.id, 'id');
  if (!TYPE_PAIRS[record.memory_type]?.includes(record.record_type))
    throw new MemoryError('Invalid memory_type/record_type pair.');
  assertScope(record, settings);
  stringValue(record.topic, 'topic');
  const summary = stringValue(record.summary, 'summary');
  if (summary.length > 1000) throw new MemoryError('summary exceeds 1000 characters.');
  if (record.details !== null && typeof record.details !== 'string')
    throw new MemoryError('details must be string or null.');
  validateSource(record.source, record.confidence);
  validateTimestamp(record.created_at);
  stringValue(record.created_by, 'created_by');
  if (record.status !== 'active') throw new MemoryError('status must be active.');
  if (!Array.isArray(record.supersedes) || !record.supersedes.every((id) => ULID_PATTERN.test(id)))
    throw new MemoryError('supersedes must contain ULIDs.');
  if (record.supersedes.includes(record.id)) throw new MemoryError('record cannot supersede itself.');
  if (!Array.isArray(record.tags) || !record.tags.every((tag) => typeof tag === 'string' && tag.length > 0))
    throw new MemoryError('tags must contain non-empty strings.');
  scanSecrets(record);
}

function validateTombstone(value: unknown, settings: MemorySettings): asserts value is MemoryTombstone {
  if (!isMapping(value)) throw new MemoryError('Memory tombstone must be a mapping.');
  const tombstone = value as unknown as MemoryTombstone;
  if (tombstone.schema_version !== 1) throw new MemoryError('Unsupported tombstone schema version.');
  assertUlid(tombstone.id, 'id');
  assertUlid(tombstone.target_id, 'target_id');
  assertScope(tombstone, settings);
  stringValue(tombstone.reason, 'reason');
  validateSource(tombstone.source, 'confirmed');
  validateTimestamp(tombstone.created_at);
  stringValue(tombstone.created_by, 'created_by');
  scanSecrets(tombstone);
}

function validateSource(source: MemorySource, confidence: Confidence): void {
  if (!isMapping(source)) throw new MemoryError('source must be a mapping.');
  if (!['artifact', 'user-confirmed', 'discussion', 'tool-observation'].includes(source.kind))
    throw new MemoryError('Invalid source.kind.');
  if (source.ref !== null && typeof source.ref !== 'string')
    throw new MemoryError('source.ref must be string or null.');
  if (source.revision !== null && typeof source.revision !== 'string')
    throw new MemoryError('source.revision must be string or null.');
  if (confidence === 'verified' && !['artifact', 'tool-observation'].includes(source.kind))
    throw new MemoryError('verified confidence requires artifact or tool-observation source.');
}
...
function validateTimestamp(value: unknown): void {
  if (
    ...
    throw new MemoryError('created_at must be an RFC 3339 UTC timestamp.');
}
