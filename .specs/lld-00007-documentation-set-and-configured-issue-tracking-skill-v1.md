...
- `.specs/lld-00001-generic-configuration-tools-and-harness-adapters-v1.md` for configuration ownership, default overlay behavior, and host adapters.
- `.specs/lld-00002-caveman-and-memory-skills-v1.md` for skill compilation, repository memory, security, and OpenCode/Pi boundaries.
- `.specs/lld-00002-caveman-memory-hooks-across-sdlc-commands-v2.md` for current SDLC command integration, exact installer rollback, and release-artifact verification.
- `.specs/lld-00005-canonical-yaml-issue-storage-v1.md` for issue lineage, subject to its supersession notice.
- `.specs/lld-00006-simplified-local-persistence-and-sqlite-write-through-cache-v1.md` as the authoritative current local issue, memory, and cache contract.
...
- Persisting repository coordinates, credentials, tokens, CLI login state, or command arguments in harnessctl configuration.
...
| `issues.root` | Safe project-relative path | Canonical root only when type is filesystem. Retained and validated for overlay compatibility when type is remote, but not used remotely. |
| `issues.prefix` | ASCII letters, digits, underscores, or hyphens | Local ID prefix only when type is filesystem. Retained and validated remotely, but not used to interpret remote issue IDs. |
...
Remote providers require `issues.tools` to be explicitly present in the project input and must not inherit the filesystem default through overlay. GitHub requires exactly `gh`, GitLab exactly `glab`, and Gitea exactly `tea`, after normalization. Known provider executables are mutually exclusive: configurations such as GitHub with `glab`, GitLab with `gh`, or Gitea with either are rejected rather than rendered generically. Forgejo has no verified official CLI contract in this design and therefore requires exactly one explicitly configured safe executable identifier. It may be an operator-selected compatibility client, including `tea`, but receives no assumed capability or syntax. These fail-closed rules prevent local-tool leakage, mixed-provider boilerplate, and unsupported fallback chains.

Each tools entry is an identifier, not free-form shell text. Identifier validation rejects blanks, control characters, whitespace-bearing command lines, shell operators, path separators, assignments, and secret-bearing argument forms. Provider validation then enforces the exact filesystem set, the exact known-provider executable, or Forgejo's single safe executable. Harnessctl neither invokes nor probes configured executables.

Remote CLIs infer the current repository from their own current-directory behavior unless the operator configures the CLI externally. No repository owner, project slug, server URL, token, password, or credential environment-variable value is added to harnessctl configuration.
...
- A remote type without an explicitly supplied tools value is invalid even though deep overlay normally supplies defaults. The error names both `issues.type` and `issues.tools` and explains that remote tools must be selected explicitly.
- Unknown types, malformed tools, provider/executable mismatches, unsafe roots, and unsafe prefixes fail in Python, TypeScript, and the generated JSON contract where expressible.
...
- Config version does not increment because the scalar shape is unchanged, the default remains valid, and provider selection is expanded. The stricter exact-value rule intentionally invalidates previously tolerated custom filesystem subsets; migration diagnostics must name the required normalized set. Generated full configuration still includes every issue key.
...
`render_skill` receives only the validated issue provider, the normalized provider-valid tools value, and the local root and prefix when applicable. It must not receive the whole configuration document. This narrow context prevents unrelated configuration, credentials, or future secret references from entering generated Markdown.
...
All remote branches state that the configured CLI must already be installed and authenticated by the operator. Harnessctl does not execute login, accept credentials, or choose a repository. Titles retain the hierarchy emoticons from the source guidance because remote systems do not have harnessctl's local type field. Capability-specific prose is rendered only when the configured executable exactly matches its known provider contract.
...
The Forgejo branch states that no official Forgejo CLI or syntax was verified for this design. It names only the single configured executable, requires operator-provided authentication and compatibility, and tells the agent to inspect `<cli> --help` before any operation. It contains generic configured-CLI guidance only: no capability list, hardcoded subcommand, or syntax inferred from the executable's association with another provider. If the executable is `tea`, compatibility remains operator-verified and does not activate Gitea-specific prose.

Each provider branch is exclusive at render time. GitHub output contains no GitLab, Gitea, or Forgejo operational guidance; equivalent wrong-provider absence applies to every other branch. Tests may permit the configured executable itself where names overlap, such as Forgejo with `tea`, but must reject unrelated provider capability prose, boilerplate, CLI names, and syntax. Shared prose may use the generic term “remote provider.”
...
- Forgejo is explicitly recorded as having no official CLI verified for this design; no unsupported command syntax is cited or invented.
...
- `issues.tools` contains identifiers only. Credentials, tokens, passwords, repository URLs, login commands, shell fragments, and environment-variable assignments are invalid content.
- Remote authentication remains in the selected CLI's operator-managed credential store or environment. Harnessctl neither reads nor renders those values.
...
- Malformed YAML, unsupported config versions, unknown issue providers, unsafe paths, unsafe prefixes, invalid tools entries, and remote providers without explicit tools fail before installation writes.
...
- Forgejo receives no inferred `tea`, `gh`, or `glab` capability or subcommand syntax. Its rendering requires checking `<cli> --help`; configuring `tea` records only an operator-selected executable and compatibility warning.
...
| `src/harnessctl/config.py` | Provider enum, normalized exact tools validation, explicit remote-tools rule, and unchanged overlay/default behavior. |
| `extensions/generic-tools/config.ts`, `extensions/generic-tools/schemas.ts` | Matching runtime migration and canonical schema. |
...
- Accept each of the five issue types with its exact provider-valid tools value: the complete normalized filesystem set, `gh`, `glab`, `tea`, or one safe Forgejo executable.
- Require explicit tools in source configuration for every remote type and prove filesystem omission still receives the complete default list.
- Reject unknown types, blank or malformed entries, shell fragments, assignments, paths, credential-bearing command text, partial or extended filesystem sets, multiple Forgejo executables, and known-provider executable mismatches such as GitHub with `glab`.
...
- Assert no rendering contains `ISSUE_TRACKING`, `env-get`, unrendered Jinja, unrelated config, credential values, or unrelated provider boilerplate. Exclusivity assertions may allow the configured executable's name itself, including Forgejo with `tea`, while still rejecting Gitea capability and syntax prose.
...
- Assert Forgejo rendering requires `<cli> --help`, has no hardcoded subcommand or provider-specific capability prose, and labels `tea` compatibility operator-verified when selected.
...
- Assert issue docs include the verified GitHub, GitLab, and Gitea facts and the unverified-Forgejo warning without unsupported syntax.
...
7. Filesystem requires exactly the normalized complete harnessctl issue-tool set. GitHub requires `gh`, GitLab `glab`, and Gitea `tea`; mismatched known-provider executables are rejected. Forgejo requires one safe explicit executable. No configuration contains credentials or silently inherits local tools.
...
9. Capability-specific prose appears only for the exact configured known executable/provider pair. Forgejo output requires `<cli> --help`, makes no official-CLI, capability, or hardcoded-subcommand claim, and labels any `tea` compatibility operator-verified.
...
- False Forgejo confidence: configured-client compatibility may vary. Mitigation: one explicit safe executable, generic guidance, mandatory `<cli> --help`, no inferred capabilities or subcommands, and an operator-verified warning.
...
- A richer structured `issues.tools` object may be considered only if one exact provider capability selection proves insufficient; version 2 keeps the existing scalar shape.
- Remote repository coordinates remain CLI-owned unless a future design demonstrates a portable, credential-safe need.
