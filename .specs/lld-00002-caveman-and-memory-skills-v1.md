...
Root `contracts/` is the only authored schema source. Release build copies exact bytes into Python package data (`harnessctl/contracts/`) and a new `@harnessctl/contracts` npm package. Build fails unless SHA-256 hashes match root sources. Python loads schema through `importlib.resources`; TypeScript imports it from `@harnessctl/contracts`. Both use permissively licensed YAML/JSON Schema validators. Shared valid/invalid fixtures prevent parser drift. Neither runtime maintains a handwritten duplicate.
...
| File | Change |
| --- | --- |
| `contracts/config-v2.schema.json` | Only authored config schema source. |
| `contracts/memory-record-v1.schema.json` | Only authored record/tombstone schema source. |
| `extensions/contracts/` | Publish exact schema bytes as `@harnessctl/contracts`. |
...
| `src/harnessctl/contracts/` | Wheel package-data copies generated byte-for-byte from root contracts. |
