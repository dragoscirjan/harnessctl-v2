...
Root `contracts/` is the only schema source. Initial runtime validators implement the same rules directly and use shared valid/invalid fixtures to detect behavior drift. Schemas remain repository-level contracts until a runtime needs to load them after installation. If that requirement appears, packaging copies must be generated during the release build rather than committed as duplicate source files.
