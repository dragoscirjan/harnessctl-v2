...
- Before web search or fetch, prefer live tools under `sdlc_web_crawl` when available and suitable. Inspect schemas first; use search/fetch/stash/grep capabilities before `curl`, `wget`, or ad hoc shell fetch. Fall back when disabled, unavailable, stale, incomplete, or unsuitable.
