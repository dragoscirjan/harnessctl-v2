...
- Scope one current repository. Familiarize from current issues, specs, source, Git state, tests,
  configuration, and provider observations. Read only what supports bounded reconciliation; never
  read secrets.
- Treat source authority as newer than memory or projections. Do not mutate issues, specs, source,
